...
// Ported verbatim from CASE_STUDIES in App.jsx (line ~25191).
...
const RAW_CASE_STUDIES: Omit<CaseStudy, "categorySlug">[] = [
  {
    id: "cs_01",
    ...
    duration: "12 days",
    scope:
      "Complete turnkey overhaul: rotor removal, bearing replacement, labyrinth seal machining, rotor dynamic balancing (ISO 1940 G2.5), lube oil flushing, and commissioning.",
    challenge:
  ...
  },
  {
    ...
    duration: "21 days",
    scope:
      "3D laser scan of damaged impulse disc, PMI material verification, full manufacturing drawing set, and supply of replacement disc machined to drawing.",
    challenge:
  ...
  },
  {
    ...
    duration: "28 days",
    scope:
      "Complete erection of steam turbine, gearbox, alternator, condenser, lube oil system, and all associated piping and cable work. OEM witness commissioning.",
    challenge:
  ...
  },
  {
    ...
    duration: "5 days",
    scope:
      "Post-overhaul lube oil system flushing using mobile centrifuge filter unit. Target cleanliness: ISO 4406 ≤16/14/11 for main lube oil circuit and ≤15/13/10 for control oil circuit.",
    challenge:
  ...
  },
  {
    ...
    duration: "2 days",
    scope:
      "Precision laser alignment of a three-element compressor train (motor–gearbox–compressor) following gearbox replacement. Pipe strain measurement and correction.",
    challenge:
  ...
  },
  {
    ...
    duration: "4 days",
    scope:
      "Workshop dynamic balancing of a 680 kg ID fan rotor to ISO 21940-11 G2.5 quality grade, plus journal grinding and polishing on both drive-end and non-drive-end journals.",
    challenge:
  ...
  },
  {
    ...
    scope:
      "Emergency site response, root-cause diagnosis, replacement babbitt bearing manufacture and fit, rotor inspection, and recommissioning within a 14-hour window.",
    challenge:
...
// ─── CATEGORY SLUG (stable cross-reference key, decoupled from the ─────────
// translatable `category` display label — matches SVC_CATEGORY_MAP in
// service-details.ts, see i18n Phase 2 corrections) ─────────────────────────
...
export const CASE_STUDIES: CaseStudy[] = RAW_CASE_STUDIES.map((cs) => ({
  ...cs,
  categorySlug: slugify(cs.category),
}));
